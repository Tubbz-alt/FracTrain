...
def quantize(x, num_bits=8, min_value=None, max_value=None, num_chunks=None, stochastic=False, inplace=False):
    if num_bits:
        return UniformQuantize().apply(x, num_bits, min_value, max_value, stochastic, inplace, False, num_chunks)
    else:
        return x
...
class QuantMeasure(nn.Module):
    ...
    def forward(self, input, num_bits):
        ...
        return quantize(input, num_bits, min_value=float(min_value), max_value=float(max_value))


class QConv2d(nn.Conv2d):

    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=False, momentum=0.9, quant_act_forward=0, quant_act_backward=0, quant_grad_act_error=0, quant_grad_act_gc=0, weight_bits=0, fix_prec=False):
        super(QConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        self.quantize_input_fw = QuantMeasure(momentum=momentum)
        self.quantize_input_bw = QuantMeasure(momentum=momentum)
    ...
    def forward(self, input, num_bits, num_grad_bits):
        ...
        if self.fix_prec:
            if self.quant_act_forward or self.quant_act_backward or self.quant_grad_act_error or self.quant_grad_act_gc or self.weight_bits:
                qweight = quantize(self.weight, num_bits=self.weight_bits,
                ...
                qweight = quantize(self.weight, num_bits=num_bits,
                               min_value=float(self.weight.min()),
                               max_value=float(self.weight.max()))
...
if __name__ == '__main__':
    ...
    quantize_input1 = QuantMeasure()
    quantize_input2 = QuantMeasure()

    out1 = quantize_input1(input, num_bits=8)
    out2 = quantize_input2(input, num_bits=8)
