...
# bits = [3, 4, 4, 6, 8, 8]
# grad_bits = [6, 6, 8, 8, 8, 12]

# bits = [3, 4, 4, 6, 6, 8]
# grad_bits = [6, 6, 8, 8, 12, 12]

# bits = [3, 4, 6, 8, 0]
# grad_bits = [6, 8, 12, 16, 0]

# bits = [3, 4, 4, 6, 6, 6, 8]
# grad_bits = [6, 6, 8, 8, 10, 12, 16]

bits = [4, 6, 8, 6, 8]  
grad_bits = [6, 8, 8, 12, 16]
...
def run_training(args):
    ...
    # create model
    model = models.__dict__[args.arch](args.pretrained, proj_dim=len(bits)+1)
    ...
    for name, param in model.named_parameters():
        if 'gate' in name or 'control' in name:
            var_rnn_gate.append(param)
        else:
            var_backbone.append(param)

    optimizer1 = torch.optim.SGD(var_backbone,
                                args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)

    optimizer2 = torch.optim.SGD(var_rnn_gate,
                                args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)
    ...
    while i < args.iters + args.finetune_step:
        for input, target in train_loader:
            ...
            # if computation_cost > loss_cls/10:
            #     args.beta *= args.beta_decay

            if computation_cost > loss_cls/10 and args.ada_beta: 
                computation_cost *= loss_cls.detach()/10/computation_cost.detach()
...
def adjust_learning_rate(args, optimizer1, optimizer2, _iter):
    if args.lr_schedule == 'piecewise':
        if args.warm_up and (_iter < 400):
            lr = 0.01
        elif 32000 <= _iter < 48000:
    ...
    for param_group in optimizer2.param_groups:
        param_group['lr'] = lr/3 if _iter >= 5000 else lr
